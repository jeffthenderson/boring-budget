...
function buildBookmarklet(baseUrl: string) {
  ...
  const buildPageQueue=(doc,current,totalCount,step)=>{const base=new URL(current,location.origin);const currentStart=Number.parseInt(base.searchParams.get('startIndex')||'0',10)||0;const queue=[];if(totalCount&&step){const totalPages=Math.max(1,Math.ceil(totalCount/step));const maxStart=(totalPages-1)*step;for(let start=0;start<=maxStart;start+=step){if(start===currentStart)continue;const url=new URL(base.toString());url.searchParams.set('startIndex',String(start));queue.push(url.toString());}return queue;}const links=Array.from((doc.querySelector('.a-pagination')||doc).querySelectorAll('a'));const startIndices=new Set();const pages=new Set();for(const link of links){const href=link.getAttribute('href');if(!href)continue;try{const url=new URL(href,location.origin);const startParam=url.searchParams.get('startIndex');if(startParam!==null&&startParam!==''){const start=Number.parseInt(startParam,10);if(Number.isFinite(start))startIndices.add(start);}const pageParam=url.searchParams.get('page');if(pageParam!==null&&pageParam!==''){const page=Number.parseInt(pageParam,10);if(Number.isFinite(page))pages.add(page);}}catch{}}const currentPage=Number.parseInt(base.searchParams.get('page')||'1',10)||1;if(startIndices.size){const sorted=[...startIndices].sort((a,b)=>a-b);let stepSize=Infinity;for(let i=1;i<sorted.length;i+=1){const diff=sorted[i]-sorted[i-1];if(diff>0&&diff<stepSize)stepSize=diff;}if(!Number.isFinite(stepSize)||stepSize<=0)stepSize=10;const maxStart=sorted[sorted.length-1];const startAt=sorted[0];for(let start=startAt;start<=maxStart;start+=stepSize){if(start===currentStart)continue;const url=new URL(base.toString());url.searchParams.set('startIndex',String(start));queue.push(url.toString());}return queue;}if(pages.size){const sorted=[...pages].sort((a,b)=>a-b);let stepSize=Infinity;for(let i=1;i<sorted.length;i+=1){const diff=sorted[i]-sorted[i-1];if(diff>0&&diff<stepSize)stepSize=diff;}if(!Number.isFinite(stepSize)||stepSize<=0)stepSize=1;const maxPage=sorted[sorted.length-1];const startAt=sorted[0];for(let page=startAt;page<=maxPage;page+=stepSize){if(page===currentPage)continue;const url=new URL(base.toString());url.searchParams.set('page',String(page));queue.push(url.toString());}}return queue;};
