...
function buildBookmarklet(baseUrl: string) {
  ...
  const waitForCardsForStart=async(doc,expectedStart)=>{let attempts=0;let cards=[];while(attempts<20){cards=getOrderCards(doc);const selectedStart=getSelectedStartIndex(doc);if(cards.length&& (selectedStart===null||selectedStart===expectedStart))return cards;await sleep(400);attempts+=1;}return cards;};
  const loadPageDocument=async(url,expectedStart,baselineFirstId)=>{let doc=null;let cards=[];let source='fetch';let selectedStart=null;let firstId=null;try{const res=await fetch(url,{credentials:'include',headers:{'accept':'text/html','cache-control':'no-cache','pragma':'no-cache'},cache:'no-store'});const html=await res.text();doc=new DOMParser().parseFromString(html,'text/html');cards=getOrderCards(doc);selectedStart=getSelectedStartIndex(doc);firstId=getFirstOrderIdFromCards(cards);}catch{}const expectedMismatch=expectedStart>0&&selectedStart!==null&&selectedStart!==expectedStart;const sameAsBaseline=expectedStart>0&&baselineFirstId&&firstId===baselineFirstId;const shouldUseFrame=cards.length===0||expectedMismatch||sameAsBaseline;if(shouldUseFrame){try{source='frame';doc=await loadPageInFrame(url);cards=await waitForCardsForStart(doc,expectedStart);selectedStart=getSelectedStartIndex(doc);firstId=getFirstOrderIdFromCards(cards);}catch{}}return {doc,cards,source,selectedStart,firstId};};
