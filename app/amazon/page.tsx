...
function buildBookmarklet(baseUrl: string) {
  ...
  const loadPageDocument=async(url)=>{let doc=null;let cards=[];let source='fetch';try{const res=await fetch(url,{credentials:'include',headers:{'accept':'text/html','cache-control':'no-cache','pragma':'no-cache'},cache:'no-store'});const html=await res.text();doc=new DOMParser().parseFromString(html,'text/html');cards=getOrderCards(doc);}catch{}if(cards.length===0){try{source='frame';doc=await loadPageInFrame(url);cards=await waitForOrderCardsInDoc(doc,8,400);}catch{}}return {doc,cards,source};};
